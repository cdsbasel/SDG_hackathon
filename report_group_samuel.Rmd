...
In order to so, Sustainable development goals were detected in research output with a querry based approach. aurora etc. 
...
Only Publications and Projects that were published or completed from 2010 to 2020 were included.The Query search used was Aurora (Link) and Elsevier (Link). Abstracts for all publications were identified using corpustools R package. About 85% of abstracts could be retrieved.SDG
...
# Distribution of SDG
Text including numbers computed using inline code: No. articles (`r toString(n_publications)`), No. of projects (`r toString(n_projects)`), No. of matches (any SDG) in both (publications: `r toString(n_matches_publications)`; projects: `r toString(n_matches_projects)`). 

Figure showing frequency of SDG matches by SDG, output type (publication, self-funded project, externally-funded project)

The figure below shows the number of publications related to SDGs at the University of Basel. Of the `r toString(n_projects)` publications a total of `r toString(n_matches_publications)` matched at least one of the SDGs. 
...
The figure below shows the number of projects dealing with the SDGs. Externally funded projects are in a lighter hue and internally funded projects are in the darker hue. Again out of the `r toString(n_projects)` projects concerning SDGs SDG-03 receives the most attention with a total of `r sum(pro2$`n`[pro2$name== "SDG-03"])` projects.
